...
function feedbacks(system) {
	...
	function feedback_check_constraints(constraint, feedback) {
		if (constraint) {
			...
			if (constraint.feedback_types && constraint.feedback_types.length !== 0 && !constraint.feedback_types.includes(feedback.type)) {
				return false
			}
			// check if limited to feedback ids
			if (constraint.feedback_ids && constraint.feedback_ids.length !== 0 && !constraint.feedback_ids.includes(feedback.id)) {
				return false
			}
		}
		return true
	...
	system.on('feedback_check_constraints', function (constraints, feedback, cb) {
		cb(feedback_check_constraints(constraints, feedback))
	})
		...
		if (self.feedbacks[page][bank] !== undefined) {
			system.emit('get_bank', page, bank, function (bank_obj) {
				...
				for (var i in self.feedbacks[page][bank]) {
					...
					if (!feedback_check_constraints(constraint, feedback)){
						continue
					}
			...
			if (feedbacks !== undefined) {
				for (var n in feedbacks) {
					...
					if (feedback !== undefined && feedback.id === feedbackid) {
						...
						self.system.emit('feedback_save')
						self.system.emit('feedback_check_bank', page, bank, { instance_id: feedback.instance_id, feedback_ids: [feedback.id] })
						self.subscribeFeedback(feedback)
					}
				}
			}
...
feedbacks.prototype.subscribeFeedback = function (feedback) {
	var self = this
...
feedbacks.prototype.unsubscribeFeedback = function (feedback) {
	var self = this
...
feedbacks.prototype.setCachedStyle = function (page, bank, index, definition, feedback, style) {
	var self = this
