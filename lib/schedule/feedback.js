...
const feedbacks_manager = require('../feedback')
...
class feedback extends plugin_base {
	...
	check_feedbacks(constraints) {
		console.log('do check', constraints)
	...
	check_entry(entry, constraints) {
		...
		for (const feedback of feedbacks) {
			...
			if (last_value !== undefined) {
				...
				let allow = false
				self.scheduler.system.emit('feedback_check_constraints', constraints, feedback, (a) => allow = a)
				if (!allow){
					continue
				}
			}
